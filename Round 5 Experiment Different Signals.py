...
class OnlineDirectionModel:
    def __init__(self, feature_dim=4, learning_rate=0.01): #0.008->0.01
...
class Trader:
    def __init__(self):
        ...
        self.products_ml = [
            ...
            "VOLCANIC_ROCK_VOUCHER_10500","CROISSANTS","MAGNIFICENT_MACARONS"
        ]
        self.ml_signal_config = {
            "JAMS": {"high": 0.90, "low": 0.10}, #Changed
            "DJEMBES": {"high": 0.90, "low": 0.10}, #Changed .9->.85 and .1->.15
            "VOLCANIC_ROCK": {"high": 0.80, "low": 0.20}, #Changed .8->.9 and .2->.1
            "VOLCANIC_ROCK_VOUCHER_9500": {"high": 0.55, "low": 0.45},
            "VOLCANIC_ROCK_VOUCHER_9750": {"high": 0.55, "low": 0.45},
            "VOLCANIC_ROCK_VOUCHER_10000": {"high": 0.55, "low": 0.45},
            "VOLCANIC_ROCK_VOUCHER_10250": {"high": 0.55, "low": 0.45},
            "VOLCANIC_ROCK_VOUCHER_10500": {"high": 0.55, "low": 0.45},
            "CROISSANTS": {"high": 0.85, "low": 0.15}, #added
            "MAGNIFICENT_MACARONS": {"high": 0.55, "low": 0.45}, #added
        }

        self.products_stat = ["KELP", "RAINFOREST_RESIN"] #Removed "MAGNIFICENT_MACARONS"
        self.products_zscore = {
            #"CROISSANTS": {"short_window": 12, "long_window": 80, "z_entry": 1.8, "volume": 15}, #was 8,100
            "SQUID_INK": {"short_window": 8, "long_window": 80, "z_entry": 2, "volume": 15} #was 10,100,3
        }
        ...
        self.position_limits = {
            "CROISSANTS": 250, "JAMS": 350, "DJEMBES": 60, "SQUID_INK": 0,
            "KELP": 50, "RAINFOREST_RESIN": 50,
            "PICNIC_BASKET1": 60, "PICNIC_BASKET2": 0,
            "VOLCANIC_ROCK": 400,
    ...
    def run(self, state: TradingState):
        ...
        for product in self.all_products:
            ...
            # --- STAT-ARB STRATEGY ---
                ...
                fair_value += 0.1 * momentum + 0.5 * imbalance
            ...
            # --- ML STRATEGY (now product-specific thresholds) ---
                ...
                thresholds = self.ml_signal_config.get(product, {"high": 0.55, "low": 0.45}) #Lowered
        ...
        traderData = jsonpickle.encode({
            "feature_histories": {k: v[-120:] for k, v in self.feature_histories.items()},
            "price_histories": {k: v[-120:] for k, v in self.price_histories.items()},
